...
def test_prev_hash_chains_correctly(chain):
    ...
    # r2's prev_hash must equal the hash of r1's full receipt
...
def test_tamper_detection(chain):
    ...
    # Tamper with the first receipt's action field
...
def test_ts_is_integer_epoch_ms(chain):
    ...
    assert r["ts"] > 1_700_000_000_000   # sanity: after Nov 2023
...
def test_to_agentreceipts_v1_iso_timestamp(chain):
    ...
    # ts must now be an ISO 8601 string
